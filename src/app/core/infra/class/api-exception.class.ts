...
import type { ApiErrorBase, ApiFailedResponse } from '@/domain/models/app/api-core.models';
...
export class ApiExceptionCore {
  static createError<T extends ApiErrorBase = ApiErrorBase>(response: ApiFailedResponse<T>) {
    const { code, message } = response.error;
    const ExceptionClass = ApiErrorMap.get(code as ApiErrorCodesEnum);
